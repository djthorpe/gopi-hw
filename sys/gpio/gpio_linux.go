...
func (config GPIO) Open(logger gopi.Logger) (gopi.Driver, error) {
	logger.Debug("<hw.gpio.linux>Open{ UnexportOnClose=%v }", config.UnexportOnClose)
...
func (this *gpio) Close() error {
	this.log.Debug("<hw.gpio.linux>Close{ }")
	this.Lock()
	defer this.Unlock()
	...
	for pin, file := range this.watched {
		...
		if err := file.Close(); err != nil {
			this.log.Warn("<hw.gpio.linux>Close: %v: %v", pin, err)
		}
	}
		...
		for _, pin := range this.exported {
			if isExported(pin) {
				if err := unexportPin(pin); err != nil {
					this.log.Warn("<hw.gpio.linux>Close: Unable to export pin %v: %v", pin, err)
				}
			}
		}
	}
...
func (this *gpio) ReadPin(pin gopi.GPIOPin) gopi.GPIOState {
	this.log.Debug2("<hw.gpio.linux>ReadPin{ pin=%v }", pin)
	this.Lock()
	defer this.Unlock()
...
func (this *gpio) WritePin(pin gopi.GPIOPin, state gopi.GPIOState) {
	this.log.Debug2("<hw.gpio.linux>WritePin{ pin=%v state=%v }", pin, state)
	this.Lock()
	defer this.Unlock()
...
func (this *gpio) GetPinMode(pin gopi.GPIOPin) gopi.GPIOMode {
	this.log.Debug2("<hw.gpio.linux>GetPinMode>{ pin=%v }", pin)
	this.Lock()
	defer this.Unlock()
...
func (this *gpio) SetPinMode(pin gopi.GPIOPin, mode gopi.GPIOMode) {
	this.log.Debug2("<hw.gpio.linux>SetPinMode>{ pin=%v mode=%v }", pin, mode)
	this.Lock()
	defer this.Unlock()
...
func (this *gpio) Watch(pin gopi.GPIOPin, edge gopi.GPIOEdge) error {
	this.log.Debug2("<hw.gpio.linux>Watch{ pin=%v edge=%v }", pin, edge)
	this.Lock()
	defer this.Unlock()
...
func (this *gpio) Emit(pin gopi.GPIOPin, edge gopi.GPIOEdge) {
	this.Publisher.Emit(&gpio_event{driver: this, pin: pin, edge: edge})
}
...
func (this *gpio) String() string {
	...
	return fmt.Sprintf("<hw.gpio.linux>{%v }", str)
}
